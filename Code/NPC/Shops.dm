...
proc/npcsay(T as text)
	return "<font color=#FFFFFF>[T]</font>"
...
mob/TalkNPC/male_wigseller
	...
	Talk()
		set src in oview(4)
		usr << npcsay("<font color = red><b>Wig Seller </b></font>: Welcome to our little wig shop, use the mirrors to select a wig of your liking!")
...
mob/TalkNPC/Tammie
	...
	Talk()
		set src in oview(2)
		usr << npcsay("<font color = red><b>Tammie </b></font>: Sorry, sweetie. I fell over and hurt my leg so now I can't reach any of my merchandise! What a shame!")
		return
		if(usr.Year in list("1st Year","2nd Year","3rd Year",""))
			usr << npcsay("<font color = red><b>Tammie </b></font>: Sorry, sweetie. Come back when you're older.")
		else
			...
			switch(input("Tammie : What can I get for ya, hun?","You have [comma(usr.gold)] gold")as null|anything in list("Draft Beer  50g","Iced Tea  50g","Cocoa Nut Cream Pie  80g","Blueberry Pie  80g","Apple Pie  80g"))
				...
				if(null)
					usr << npcsay("<font color = red><b>Tammie </b></font>: Just gimme a holler if ya want anything, darlin'.")
					return
			if(usr.gold < selectedprice)
				usr << npcsay("<font color = red><b>Tammie </b></font>: Ya don't have enough coin for that darlin'. It costs [selectedprice]g.")
			else
				usr.gold -= selectedprice
				ministrybank += taxrate*selectedprice/100
				new selecteditem(usr)
				usr << npcsay("<font color = red><b>Tammie </b></font>: There ya go darl. Enjoy.")
		...
		switch(input("Tom : Welcome to the Leaky Cauldron. What do ya wanna do?","You have [comma(usr.gold)] gold")as null|anything in list("Shop","Talk"))
			if("Talk")
				...
				if("Rats in the Cellar" in p.questPointers)
					...
					if(pointer.stage == 1)
						usr << npcsay("<font color = red><b>Tom </b></font>: I will take you down to the cellar")
						sleep(20)
						if(get_dist(usr,src)<5)
							p.Transfer(locate(64,27,26))
							var/mob/TheTom = new/mob/Tom_(locate(65,27,26))
							usr << npcsay("<font color = red><b>Tom </b></font>: This is as far as I'll go. Thank you again.")
							sleep(50)
							del TheTom
						else
							usr << npcsay("You hear a voice in your head: <i>Hey! You're not even here!</i>")
					else if(pointer.stage == 2)
						usr << npcsay("<i>Tom stares at you in awe.</i>")
						sleep(10)
						usr << npcsay("<font color = red><b>Tom </b></font>: You did it!")
						sleep(10)
						usr << npcsay("<font color = red><b>Tom </b></font>: Thank you so much! I don't have much to give you, but I can give you this.")
						p.checkQuestProgress("Tom")
					else
						usr << npcsay("<font color = red><b>Tom </b></font>: Hey, Thanks again!")

				else
					usr << npcsay("<font color = red><b>Tom </b></font>: Hello there, Professor Palmer sent me an owl about you.")
					switch(input("Tom: Did you come to help me?","Help?")in list("Yes","No"))
						if("Yes")
							usr << npcsay("<font color = red><b>Tom </b></font>: Excellent!")
							sleep(30)
							usr << npcsay("<font color = red><b>Tom </b></font>: I always store my beverages in a cellar below the ground in order to keep them cool, however recently there have been rats somehow getting in.")
							sleep(30)
							usr << npcsay("<font color = red><b>Tom </b></font>: If you would be so kind as to get rid of all the rats, and stop them from getting back in. I would be very greatful.")
							switch(input("Tom: What do you say?","Help?")in list("Yes","No"))
								if("Yes")
									usr << npcsay("<font color = red><b>Tom </b></font>: Thank you so much.")
									sleep(20)
									usr << npcsay("<font color = red><b>Tom </b></font>: I will take you down to the cellar")
									sleep(40)
									if(get_dist(usr,src)<5)
										p.startQuest("Rats in the Cellar")
										usr.loc = locate(64,27,26)
										var/mob/TheTom = new/mob/Tom_(locate(65,27,26))
										usr << npcsay("<font color = red><b>Tom </b></font>: This is as far as I'll go. Thank you again.")
										sleep(50)
										del TheTom
									...
									return
								if("No")
									usr << npcsay("<font color = red><b>Tom </b></font>: Alright then.")
									return
						if("No")
							usr << npcsay("<font color = red><b>Tom </b></font>: My mistake. Carry on.")
							return
			if("Shop")
				if(usr.Year in list("1st Year","2nd Year","3rd Year",""))
					usr << npcsay("<font color = red><b>Tom </b></font>: Sorry mate. Come back when you're older.")
				else
					var/obj/selecteditem
					var/selectedprice
					switch(input("Tom: What can I get for ya?","You have [comma(usr.gold)] gold")as null|anything in list("Draft Beer  50g","Iced Tea  50g"))
						...
						if("Iced Tea  50g")
							...
							selecteditem = /obj/Tea
						if(null)
							usr << npcsay("<font color = red><b>Tom </b></font>: Just gimme a holler if ya want anything.")
							return
					if(usr.gold < selectedprice)
						usr << npcsay("<font color = red><b>Tom </b></font>: Ya don't have enough coin for that. I ain't budging on these prices - it's [selectedprice]g or nothing.")
					else
						usr.gold -= selectedprice
						ministrybank += taxrate*selectedprice/100
						new selecteditem(usr)
						usr << npcsay("<font color = red><b>Tom </b></font>: There ya go. Enjoy.")
						usr:Resort_Stacking_Inv()
			if(null)
				usr << npcsay("<font color = red><b>Tom </b></font>: Seeya later then.")
...
mob/TalkNPC/Divo
	...
	Talk()
		...
		var/itemlist
		if(magicEyesLeft)
			itemlist = list("Magical Eye 10,000,000g","Invisibility Cloak 1,000,000g")
		else
			itemlist = list("Invisibility Cloak 1,000,000g")
		switch(input("Divo: Hi there! Welcome to Divo's Magical Wares. [magicEyesLeft ? "I've got a limited supply of these ultra rare magical eyes. They let you see invisible people. Very powerful stuff. I'm only going to sell a limited amount though, otherwise my cloak business would be pointless wouldn't it? I have [magicEyesLeft] left." : ""]","You have [comma(usr.gold)] gold")as null|anything in itemlist)
			...
			if("Invisibility Cloak 1,000,000g")
				selectedprice = 1000000
				selecteditem = /obj/items/wearable/invisibility_cloak
			if(null)
				usr << npcsay("<font color = red><b>Divo </b></font>: Sorry that I don't have anything interesting to ya... ")
				return
		if(usr.gold < selectedprice)
			usr<< npcsay("<font color = red><b>Divo </b></font>: I'm running a business here - you don't have enough gold for this.")
		else
			...
			ministrybank += taxrate*selectedprice/100
			new selecteditem(usr)
			usr << npcsay("<font color = red><b>Divo </b></font>: Thank you.")
			usr:Resort_Stacking_Inv()
...
mob/TalkNPC/Blotts
	...
	Talk()
		...
		switch(input("Blotts: Hi there! Welcome to Flourish and Blotts. Do any titles interest you?","You have [comma(usr.gold)] gold")as null|anything in list("Basic COMC 500g","Monster book of Monsters 500g"))
			...
			if(null)
				usr << npcsay("<font color = red><b>Blotts </b></font>: Come see me any time if you change your mind.")
				return
		if(usr.gold < selectedprice)
			usr << npcsay("<font color = red><b>Blotts </b></font>: Unfortunately you don't have enough for this book - it's [selectedprice]g.")
		else
			usr.gold -= selectedprice
			ministrybank += taxrate*selectedprice/100
			new selecteditem(usr)
			usr << npcsay("<font color = red><b>Blotts </b></font>: Thanks very much for your business!")
			usr:Resort_Stacking_Inv()
				...
				if("Yes")
					...
					switch(alert("This [wandname] costs 100 gold. Would you like to purchase it?","You have [comma(usr.gold)]","Yes","No"))
						if("Yes")
							if(usr.gold>=100)
								var/mob/Player/p = usr
								p.checkQuestProgress("Acquire wand")
							...
							switch(alert("This [wandname] costs 100 gold. Would you like to purchase it?","You have [comma(usr.gold)]","Yes","No"))
								if("Yes")
									if(usr.gold>=100)
										...
										usr.gold -= 100
										var/mob/Player/p = usr
										if(p.checkQuestProgress("Acquire wand"))
											p.startQuest("Hogwarts arrival")
	...
	verb
		Heal_Me()
			if(canUse(usr,cooldown=/StatusEffect/UsedFerulaToHeal))
				set src in oview(1)
				usr<<"<b><font color = red>Madam Pomfrey </b>:</font><font color=aqua> Episkey [usr]!"
	...
	Madame_Pomfrey//Names the NPC//Do i really need to say... Sets their ICON STATE
		...
		Click()//This starts- wait... you know what this is... i hope ^^
			switch(input("Would you like me to heal you?","Madam Pomfrey the Nurse")in list("Yes, Please.","Cure my illness.","No Thanks."))
				if("Yes, Please.")
					usr<<"<b><font color = red>Madam Pomfrey </b>:</font><font color=aqua> Episkey!"
					usr.overlays+=image('attacks.dmi',icon_state="heal")
					usr.HP=usr.MHP+usr.extraMHP
					usr.updateHPMP()
					sleep(10)
					usr.overlays-=image('attacks.dmi',icon_state="heal")
				if("Cure my illness.")
					usr<<"<b><font color = red>Madam Pomfrey </b>:</font> Ah, I shall use an ancient remedy to cure your afflictions."
					sleep(20)
					usr<<"<b><font color = red>Madam Pomfrey </b>:</font><font color = aqua> An Mani Elem, Vas Rel Por!"
					...
					usr.Zitt=0
				if("No Thanks.")
					usr<<"<b><font color = red>Madam Pomfrey </b></font>: Very well then. Off you go."
	verb
		Heal_Me()
			set src in oview(1)
			usr<<"<b><font color = red>Madam Pomfrey </b>:</font><font color = aqua> Episkey [usr]!"
		...
		switch(input("Would you like me to heal you?","Madam Pomfrey the Nurse")in list("Yes, Please.","No Thanks."))
			if("Yes, Please.")
				...
				spawn(10)
					usr.overlays-=image('attacks.dmi',icon_state="heal")
			if("No Thanks.")
				usr<<"<font color = red><b>Madam Pomfrey </b></font>: Very well then. Off you go."
	verb
		Heal()
			set src in oview(1)
			...
			switch(input("Would you like me to heal you?","Madam Pomfrey the Nurse")in list("Yes, Please.","No Thanks."))
				if("Yes, Please.")
					usr<<"<b><font color = red>Madam Pomfrey :</font><font color = aqua> Episkey!"
					usr.overlays+=image('attacks.dmi',icon_state="heal")
					usr.HP=usr.MHP+usr.extraMHP
					usr.updateHPMP()
					sleep(10)
...
mob/TalkNPC/Broom_Salesman
	...
	Talk()
		...
		var/obj/selecteditem
		var/selectedprice
		switch(input("Chrono : Hi there! Welcome to Chrono's Brooms. We have two models in stock right now - would you like to purchase one?","You have [comma(usr.gold)] gold")as null|anything in list("Cleansweep Seven - 50,000g","Nimbus 2000 - 100,000g"))
			if("Cleansweep Seven - 50,000g")
				selecteditem = /obj/items/wearable/brooms/cleansweep_seven
				selectedprice = 50000
			if("Nimbus 2000 - 100,000g")
				selectedprice = 100000
				selecteditem = /obj/items/wearable/brooms/nimbus_2000
			if(null)
				usr << npcsay("<font color = red><b>Chrono </b></font>: Come see me any time if you change your mind.")
				return
		if(usr.gold < selectedprice)
			usr << npcsay("<font color = red><b>Chrono </b></font>: Unfortunately you don't have enough for this broom - it's [selectedprice]g.")
		else
			usr.gold -= selectedprice
			ministrybank += taxrate*selectedprice/100
			new selecteditem(usr)
			usr << npcsay("<font color = red><b>Chrono </font></b>: Thanks very much for your business, and be careful on the pitch!")
			usr:Resort_Stacking_Inv()
...
mob/Sir_Nicholas
	...
	Click()//This starts- wait... you know what this is... i hope ^^
		...
		hearers()<<"<b><font color = red>Sir Nicholas </font>:</b> G'day, [usr]. I trust your day is going well, hmm?"
		sleep(30)
...
mob/Bloody_Baron
	...
	Click()//This starts- wait... you know what this is... i hope ^^
		if(!(src in view(usr.client.view)))return
		hearers()<<"<b><font color = red>The Bloody Baron </font>:</b> When will you brats learn to leave me alone?!"
		sleep(30)
		hearers()<<"The Bloody Baron moans and mumbles profanities under his breath before flying off."
mob/Moaning_Myrtle
	icon = 'NPCs.dmi'
	icon_state = "myrtle"
	density=0
	...
	Click()//This starts- wait... you know what this is... i hope ^^
		if(!(src in view(usr.client.view)))return
		hearers()<<"<b><font color = red>Moaning Myrtle </font>:</b> Why do students keep throwing things at my head?!"
		sleep(30)
		hearers()<<"Moaning Myrtle bursts into tears and sobs loudly before drifting off."
			...
			update(i, mob/Player/M)
				...
				if(i != 0)
					...
					if(prob(10))
						...
						usr << npcsay("<b><font color = red>Magic Mirror </font></b>: <font color = #FFFFFF>[txt]</font>")
...
mob/TalkNPC/Vault_Salesman
	...
	New()
		...
		itemlist["Updated Free Vault - Free!"]                        = list("1",       0)
	...
	Talk()
		...
		if(!(usr.ckey in worldData.globalvaults) || !fexists("[swapmaps_directory]/map_[usr.ckey].sav"))
			usr << npcsay("<font color = red><b>Vault Salesman </font></b>: Please go talk to the vault master before coming to me.")
			return

		var/index = input("<font color = red><b>[name] </font></b>: Hi there! Welcome to Gringotts, perhaps you wish to purchase one of our vaults?", "You have [comma(usr.gold)] gold")as null|anything in itemlist
		if(!index)
			usr << npcsay("<font color = red><b>[name] </font></b>: I only sell to the rich! Begone!")
			return
		...
		if(v.tmpl == selectedvault&&selectedvault!=itemlist[0])
			usr << npcsay("<font color = red><b>[name] </font></b>: You already have this exact same vault.")
		else
			...
			if(usr.gold < selectedprice * 100000 || artifacts.len < selectedprice)
				usr << npcsay("<font color = red><b>[name] </font></b>: I'm running a business here - you can't afford this.")
			else
				if(usr:change_vault(selectedvault))
					...
					for(var/obj/o in artifacts)
						...
						selectedprice--
					usr:Resort_Stacking_Inv()
					usr << npcsay("<font color = red><b>[name] </b></font>: Thank you.")
...
mob/TalkNPC/Artifacts_Salesman
	...
	Talk()
		...
		switch(input("<font color = red><b>[name] </font></b>: Hello... I sell lamps and magical rarities! Now now, they're not just lamps, they're magical lamps! My lamps will help you make your wishes come true! For the right price you might also net yourself something rare!", "You have [comma(usr.gold)] gold")as null|anything in itemlist)
		...
		if(usr.gold < selectedprice * 100000 || artifacts.len < selectedprice)
			...
			for(var/obj/o in artifacts)
				...
				selectedprice--
			usr:Resort_Stacking_Inv()
			usr << npcsay("<font color = red><b>[name] </font></b>: Thank you.")
